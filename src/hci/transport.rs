...
use crate::alloc::{vec::Vec, sync::Arc};
...
pub struct TransportLayer {
    ...
    uart: Uart0,
    /// The Waker that need to be waken once new data has been arrived from the BT Host
    waker: Option<Waker>,
...
impl TransportLayer {
    ...
    /// Retrieve data from the available BT Host responses for a specific event type
    pub fn recv_response(&mut self) -> Option<HciPacket<Vec<u8>>> {
        // check the inbound channel for data that has been received
        if let Ok(packet) = self.inbound_channel.1.recv() {
            // if we received a packet, remove the waker and return the packet
    ...
    fn handle_incomming_data(this: &SharedTransportLayer) {
        ...
        this.use_for(|transport| {
            ...
            if buff[0] != 0 {
                ...
                let raw_packet = match packet_type {
                    HciPacketType::Event => {
